...
export function EventCard({ event, index }: { event: TechEvent; index: number }) {
  return (
    <Link
      to={`/events/${event.id}`}
      className="group clip-angle relative flex flex-col overflow-hidden border border-edge bg-surface transition-colors duration-300 hover:border-primary/60 focus-visible:outline-none"
    >

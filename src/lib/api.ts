...
export const api = {
  ...
  async createRegistration(input: {
    ...
    for (const eventId of input.eventIds) {
      ...
      // Solo/team events (Chess & Carrom) accept a single player or a
      // 2-player team (enforced server-side — never trust the client).
      // Only SPORT events (day-1) can have team-size requirements.
      // All Tech / Non-Tech events are individual regardless of DB config.
      if (isSportEvent(event) && isSoloTeam) {
        const maxPlayers = event.requiredPlayers ?? 2;
        if (filledPlayers < 1) throw new Error(`${event.name}: enter at least 1 player.`);
        if (filledPlayers > maxPlayers) throw new Error(`${event.name}: a maximum of ${maxPlayers} players is allowed.`);
      } else if (isSportEvent(event) && !isIndividual) {
        const requiredPlayers = event.requiredPlayers ?? 1;
        if (filledPlayers < requiredPlayers) {
          throw new Error(`${event.name}: ${requiredPlayers} players are required.`);
        }
      }
 ...
 *
 * Participants have no direct UPDATE grant on the registration tables (RLS),
 * so this goes through the `update_own_full_name` SECURITY DEFINER RPC which
 * atomically:
 *   1. updates the participant's full_name in whichever split table holds them, and
 *   2. propagates the new name into the `members[]` JSON + `captain_name` of
 *      every registration where they appear (owner/captain or listed member).
 *
 * Requires the SQL in `query_update_own_full_name.txt` to be run once in the

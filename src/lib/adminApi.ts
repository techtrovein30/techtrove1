 ...
 *
 * Calls the ensure_admin_access RPC which should promote the participant row
 * to role='admin' if the email is on the allowlist. After the RPC we
 * re-fetch the profile to verify the role was actually persisted — if it
 * wasn't, we return null so the caller can show an error instead of silently
 * bouncing the user to the public homepage.
 */
export async function adminResolveOAuthAccess(): Promise<AdminView | null> {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.user) {
    console.warn("[admin-oauth] resolve: no session");
    return null;
  }
  ...
  console.log("[admin-oauth] profile after rpc →", {
    id: profile?.id,
    email: profile?.email,
    role: profile?.role,
  });
...
export interface AdminStats {
  ...
  /** Distinct registration codes (one flat pass = one team entry). */
  totalRegistrations: number;
  /** Raw registration rows (one per event) — used for per-event bars. */
...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  // payments or revenue. A flat pass (one registration_code covering several
  // events) is billed once, so dedupe by code just like the Payments page.
  const isInternalRow = (r: RegistrationRow): boolean => {
    const members = Array.isArray(r.members) ? (r.members as Array<{ participantType?: string }>) : [];
...
export async function adminUpdateRegistrationStatusByCode(
  ...
  for (const table of ["registrations_external", "registrations_internal"] as const) {
    // A flat pass covers every event under one code, so record/clear payment on
    // the whole batch. Also clear any outstanding re-upload request — the
    // note column only exists on the external table, so never touch it there.
    const patch: { payment_status: string; payment_review_note?: string | null } = {
      payment_status: status,
    };
    if (table === "registrations_external") patch.payment_review_note = null;
...
export async function adminRequestPaymentReupload(
  ...
  const { data: current, error: fetchError } = await supabase
    .from(table)
    .select("user_id, team_name, registration_code, payment_proof_path, payment_screenshot_path, payment_screenshot_url")
    .eq("id", regId)
    .single();
  ...
  // A flat pass stores the SAME screenshot path on every row that shares the
  // registration_code. We already deleted the file from Storage, so every one
  // of those rows must be cleared too — otherwise the sibling rows keep a
  // reference to a deleted object and "View Proof" fails with a signed-URL
  // error (the "file may have been removed" bug in the admin panel).
  const { data, error } = await supabase
    .from(table)
    .update(dbUpdate)
    .eq("registration_code", (current as Record<string, unknown>).registration_code)
    .select();
  ...
  const result = rowToRegistration(data[0] as unknown as RegistrationRow);

...
/** Checks if an event is a sports event (Day 1 only — never category-based) */
export function isSportEvent(event?: TechEvent | null): boolean {
  if (!event) return false;
  return event.dayId === "day-1";
}

/**
 * Checks if an event is an individual competition.
 * ALL non-sport events (Technical / Non-Technical) are always individual —
 * only sports events can require a team.
 */
export function isIndividualEvent(event?: TechEvent | null): boolean {
  if (!event) return false;
  // Non-sport events are always individual regardless of DB config
  if (!isSportEvent(event)) return true;
  return event.registrationType === "individual" || (event.requiredPlayers ?? 1) === 1;
}

/**
 * Checks if an event supports both solo and 2-player team entries
 * (e.g. Carrom): participants may register alone or as a pair.
 */
export function isSoloTeamEvent(event?: TechEvent | null): boolean {
  if (!event) return false;
  return event.registrationType === "solo_team";
}
...
/** Validates mobile number (10-digit format) */
export function validatePhoneNumber(
  ...
  // Reject letters, spaces, or anything that isn't exactly 10 digits.

...
/**
 * Builds a public (non-signed) URL for a stored path, for cross-checking
 * whether an object still exists in Storage. Public buckets serve the file
 * directly; private buckets return a 400/403 response from Supabase, which at
 * least tells the admin whether the object is still present.
 */
export function getUploadPublicUrl(
  ...
  if (path.startsWith("http://") || path.startsWith("https://")) {
    return isTrustedStorageUrl(path) ? path : null;
  }
  ...
  if (!isSafeRelativeStoragePath(cleanPath)) return null;

      ...
      <section className="border-b border-edge bg-surface/50">
        <div className="mx-auto max-w-7xl px-4 pb-14 pt-32 sm:px-6 md:pb-16 md:pt-40 lg:px-8">
          ...
          <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted">
            Two days of competition across sports, technology and creativity. Pick a day to see
            what is on.
          </p>
          ...
          {activeDay.status === "active" && activeDay.events.length > 0 ? (
            activeDay.id === "day-2" ? (
              <div className="space-y-12">
                <div>
                  <h3 className="text-xl font-bold mb-6 text-foreground">Technical Events</h3>
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {activeDay.events.filter(e => e.category === "Technical").map((event, i) => (
                      <EventCard key={event.id} event={event} index={i} />
                    ))}
                  </div>
                </div>
                <div>
                  <h3 className="text-xl font-bold mb-6 text-foreground">Non-Technical Events</h3>
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {activeDay.events.filter(e => e.category !== "Technical").map((event, i) => (
                      <EventCard key={event.id} event={event} index={i} />
                    ))}

...
function EventModal({
  ...
  const [regType, setRegType] = useState<string>(event?.registrationType ?? "team");
  ...
  async function handleSave() {
    ...
    // Solo/team events (Chess & Carrom) always allow exactly a 2-player team.
    const isSoloTeam = regType === "solo_team";
    const effectivePlayers = isSoloTeam ? 2 : playersNum;
    const effectiveSubs = isSoloTeam ? 0 : subsNum;
    ...
    try {
      if (event) {
        await adminUpdateEvent(event.id, {
          ...
          registrationType: regType as TechEvent["registrationType"],
          registrationFee: feeNum,
          requiredPlayers: effectivePlayers,
          maxSubstitutes: effectiveSubs,
        });
      } else {
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {([
                { value: "team", label: "Team", hint: "Full team" },
                { value: "individual", label: "Individual", hint: "Solo only" },
                { value: "solo_team", label: "Solo / Team", hint: "1 or 2 players" },
              ] as const).map((opt) => (
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => {
                    setRegType(opt.value);
                    if (opt.value === "solo_team") {
                      setPlayers(2);
                      setSubs(0);
                    }
                  }}
                  className={
                    ...
                    (regType === opt.value
                      ...
                      : "border-white/10 bg-[#0f0a0a] text-muted hover:border-white/25")
                  }
                >
                  <span className="block font-semibold">{opt.label}</span>
                  <span className="block text-[9px] uppercase tracking-[0.1em] opacity-70">{opt.hint}</span>
                </button>
              ))}
            </div>
            <p className="mt-1.5 text-[10px] text-muted">
              Solo/Team (Chess & Carrom) charges the fee per participant — ₹75 solo / ₹150 for a 2-player team.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                min={1}
                value={players}
                disabled={regType === "solo_team"}
                onChange={(e) => setPlayers(Number(e.target.value))}
                className="w-full border border-white/10 bg-[#0f0a0a] px-3 py-2 text-sm text-foreground outline-none focus:border-primary/60 disabled:opacity-40"
              />
            </div>
<div>
            <label className="block text-[10px] font-semibold uppercase tracking-[0.14em] text-muted mb-1">
                        ...
                        <div className="mt-5 grid grid-cols-2 gap-3 rounded-lg border border-white/[0.04] bg-white/[0.02] p-3 text-xs">
                          <div>
                            ...
                            <p className="font-semibold text-foreground mt-0.5">
                              {event.registrationType === "solo_team"
                                ? "1 – 2 players"
                                : `${event.requiredPlayers ?? "TBA"}`}
                              {event.registrationType !== "solo_team" && (
                                <span className="text-muted font-normal text-[10px]">+ {event.maxSubstitutes ?? 0} subs</span>
                              )}
                            </p>
                          </div>
                          <div>

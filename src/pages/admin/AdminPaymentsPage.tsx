...
/** Internal registrations are free + auto-confirmed — never shown on Payments. */
...
export function AdminPaymentsPage() {
  ...
  const grouped = useMemo<BatchGroup[]>(() => {
    ...
    return Array.from(byCode.values()).map((rows) => {
      ...
      // Prefer the row that still has a screenshot attached — after a re-upload
      // request the sibling rows' paths are cleared (one file shared per code).
      ...
      return {
        ...
        proofPath:
          representative.paymentScreenshotPath ??
          representative.paymentScreenshotUrl,
        paymentStatus: rows[0].paymentStatus === "recorded" ? "recorded" : "pending",
        needsReupload,
  ...
  async function handleRequestReupload(reason: string, note: string) {
    ...
    try {
      ...
      if (updated.paymentReviewNote) {
        // The whole flat-pass batch shares one screenshot, so clear the proof
        // and flag re-upload on every row that shares the registration code.
        setRegistrations((prev) =>
          prev.map((r) =>
            r.registrationCode === reg.registrationCode
              ? {
                  ...r,
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead>
              <tr className="border-b border-white/[0.07]">
                {["Code / Team", "Event", "Captain", "Fee Amount", "UTR / Proof", "Status", "Submitted", "Action"].map(
                  (h) => (
                    <th
                      key={h}
